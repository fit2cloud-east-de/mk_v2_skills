...
def main() -> int:
    ...
    p.add_argument("--page", type=int, default=0, help="If >0 with --size, use paginated API")
    ...
    try:
        if args.page > 0:
            data = c.admin("GET", c.ws(f"application/{args.page}/{args.size}"), params=params or None)
        else:
            data = c.admin("GET", c.ws("application"), params=params or None)
        print_json(data)
        return 0
