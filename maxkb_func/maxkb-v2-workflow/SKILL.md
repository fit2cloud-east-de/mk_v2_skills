...
| Memory Feedback | [20-memory-feedback](topology-samples/20-memory-feedback.md) | session/长期记忆回流 |
...
| `publish_app.py` | 发布 |
| `debug_open.py` / `debug_chat.py` | 调试 |
...
`MAXKB_WORKSPACE` 可用显示名或 UUID。`folder_id` 默认 = workspace id。
