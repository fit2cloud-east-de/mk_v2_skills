...
创建**智能体 / 知识库 / 工具**前，**必须让用户三选一**，勿默认塞进根目录却不告知：

```markdown
资源放在哪里？（三选一）

1. **根目录** — 直接建在工作区根下，无需创建文件夹  
2. **已有文件夹** — 请告诉文件夹名称（或 id）；我会先列出可选文件夹供你选  
3. **新建文件夹** — 请给出新文件夹名称；确认后先建文件夹，再在里面创建资源  
```

| 选择 | 做法 |
|------|------|
| 根目录 | `folder_id` = workspace id（`c.folder_id()`），**不**调用建文件夹 |
| 已有文件夹 | `list_folders.py --source APPLICATION\|KNOWLEDGE\|TOOL` → 用户选定 → 创建脚本传 `--folder-id` |
