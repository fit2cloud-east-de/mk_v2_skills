...
API 对照：[api-reference.md](api-reference.md)。表述深度遵循 `MK_AUDIENCE`。
...
1. **使用 MinerU 方案**（推荐）→ 走 §1.2  
2. **仍用普通分段**（接受效果可能较差）→ `base` + `ingest_file`，并在回复中注明风险  
...
```text
数据源 → 文档解析(extract) → 分段(split) → 写入并向量化(write)  〔终结〕
```

| 步骤 | 典型节点 type | 规范 |
|------|---------------|------|
| 1 数据源 | `data-source-local-node` / `data-source-web-node` | 本地与 Web 二选一或分支；输出 `file_list` / `document_list` |
| 2 解析 | `document-extract-node` | 从文件抽出纯文本；勿跳过直接写 |
| 3 分段 | `document-split-node` | 规则与下方「分段标准」一致（limit、pattern、清洗） |
| 4 写入 | `knowledge-write-node` | 必须作为终结；触发向量化 |
...
2. 在库的工作流中按上表串好节点；**禁止**只有数据源没有 write。  
3. `POST .../knowledge/{id}/debug` 用样例文件/URL 跑通再发布。  
4. `PUT .../knowledge/{id}/publish` 后，对话侧智能体才能稳定检索该库（以实例行为为准）。  
5. 变更分段或源规则后：重新 debug → 再 publish；必要时对文档 `batch_refresh` / 整库 `PUT .../embedding`。  
...
| `delete_knowledge.py` | **删除（仅用户明确要求 + 确认删除 + `--yes`）** |
