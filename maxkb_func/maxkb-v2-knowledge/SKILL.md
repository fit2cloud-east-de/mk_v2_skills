...
| **workflow** | 工作流知识库 | 库内编排：数据源→解析→分段→写入 | **PDF + MinerU 模板**；多源清洗；通用 base「上传即分」不够用 | 简单 DOCX/MD「传几个文件」——用 base 更省事。**注意**：必须含固定「基本信息」节点 + 至少一种数据源（本地/Web/数据源工具），见 §2 |
...
需要自定义多步入库？ ──是──► workflow（自建或其它模板）
        │                      ▲ 开场先定数据从哪来：本地 / Web / 数据源工具（至少一种）
        │                      ▲ 图中必须有固定 id 的「基本信息」节点 + 写入终结
        │否
        ▼
仍优先 base
```
...
界面点「发布」时会跑 `KnowledgeWorkFlowInstance.is_valid()`（`ui/src/workflow/common/validate.ts`）。常见报错含义：

| 界面提示 | 真实含义 | 怎么修 |
|----------|----------|--------|
| **基本信息节点必填** | 图里**缺少**固定节点：`id` **必须**是 `knowledge-base-node`（`type` 同为 `knowledge-base-node`）。**不是**要你填一堆表单字段；是节点本身没了或 `id` 写错 | 补回该节点；脚本/导入时**禁止**改掉这个固定 id，也禁止用随机 uuid 代替 |
| **开始节点必填** | **没有任何数据源起点**：须至少 1 个 `properties.kind === "data-source"` 的节点 | 见下表「数据源三选一（可多）」 |
| **节点不能当做结束节点** | 某条分支的终点不是 `knowledge-write-node` | 每条数据源链路最终都要接到「知识库写入」 |

### Agent 硬规则：避免上述报错（强制）

编排 / 导入 / 保存 / 发布前必须遵守，**不要等界面报错再改**：

1. **创建 workflow 库**：默认用 `create_knowledge.py --kind workflow`（**不要**加 `--no-skeleton`）。脚本会自动写入可发布骨架：`knowledge-base-node` + 本地（或 `--skeleton web`）数据源 + 分段 + 写入。  
2. **自建 / 改图 JSON**：节点列表里**永远保留** `id=knowledge-base-node`；**至少保留一种**数据源（`kind=data-source`）；链路终点保留 `knowledge-write-node`。  
3. **保存**：只用 `save_knowledge_workflow.py`——缺基本信息时会**自动补**固定 id；若仍无数据源 / 无写入节点则**拒绝保存**（exit 2），避免把坏图写进实例。  
4. **导入 `.kbwf`**：导入后立刻 `get_knowledge.py --with-workflow` 检查是否仍有基本信息 + 数据源；被覆盖丢节点时用骨架重建或手工补回，**禁止**直接 publish。  
5. **禁止**：空 `work_flow: {}` 就让用户去界面点发布；禁止把基本信息 id 改成随机 UUID；禁止图上只有 AI/工具/分段、没有任何数据源。

**数据源起点（硬约束，开场选型就要定）**：工作流知识库**必须**至少有一类数据源，否则无法发布、也无法调试入库。可选其一或多条并行：

| 数据源 | type | `kind` | 典型场景 |
|--------|------|--------|----------|
| 本地文件 | `data-source-local-node` | `data-source` | 上传 PDF/DOCX/MD… |
| Web 站点 | `data-source-web-node` | `data-source` | 抓公开网页 |
| 数据源型工具 | `tool-lib-node` / `tool-workflow-lib-node` 且 `kind: data-source` | `data-source` | MinerU、网盘拉取等；工具本身 CRUD 见 [data-source-tools.md](../maxkb-v2-tools/data-source-tools.md) |

纯「工具 / AI / 分段」而**没有**上表任一起点 → 界面报「开始节点必填」。  
开场帮用户选 workflow 时，先问清文档从哪来（本地 / 网址 / 工具拉取），并写入设计；**不要**建一个空图只有写入节点。

另：**基本信息**节点上的「文档处理设置」`user_input_field_list` 可为空（非发布前提）；调试时若配置了必填用户字段，才需在 `knowledge_base` 里传值。

### 推荐拓扑（必须按此思路设计）

```text
[固定] knowledge-base-node（id 不可改）
[起点] 数据源（本地 / Web / 数据源工具，至少一种）
        →（可选：工具 / AI 清洗）→ 文档解析(extract) → 分段(split) → 写入(write)  〔终结〕
```

| 步骤 | 典型节点 type | 规范 |
|------|---------------|------|
| 0 基本信息 | `knowledge-base-node` | **固定 id=`knowledge-base-node`**；图中有且仅有一个 |
| 1 数据源 | `data-source-local-node` / `data-source-web-node` / **数据源型** `tool-lib-node` | **至少一种**；`properties.kind` 必须为 `data-source` |
| 2 可选加工 | `tool-lib-node` / `tool-node` / `tool-workflow-lib-node` / `ai-chat-node` 等 | **支持**自定义工具、MCP、AI；MinerU 模板即工具解析 PDF |
| 3 解析 | `document-extract-node` | 本地文件必须先提取；Web 已含 content 可跳过 |
| 4 分段 | `document-split-node` | 规则与下方「分段标准」一致 |
| 5 写入 | `knowledge-write-node` | **必须作为每条链路终结**；触发向量化 |
...
1. `create_knowledge.py --kind workflow ... --yes`（**默认带骨架**；勿随意 `--no-skeleton`）拿到知识库 id。  
2. 改图时确认：**`knowledge-base-node`（固定 id）** + **至少一种数据源** + 终点 **`knowledge-write-node`**；用 `save_knowledge_workflow.py`（自动补基本信息；无数据源则拒绝保存）。  
3. `debug_knowledge_workflow.py ... --yes` 用样例文件/URL **自动调试跑通**。  
4. `publish_knowledge.py --knowledge-id ID --yes`（或界面发布）；若仍报「基本信息/开始节点必填」→ 违反 §2「Agent 硬规则」，先 `get_knowledge.py --with-workflow` 对照。  
...
| `create_knowledge.py` | `--kind base\|web\|workflow`；**workflow 默认安装可发布骨架**（基本信息+数据源+写入），避免空图发布报错 |
