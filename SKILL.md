...
1. **编程小白（新手）** — 我不太会编程，主要懂业务。请你用大白话说明，少用术语；必要时用生活类比；要操作时给我「点哪里 / 复制哪一行」这种一步步指引。
2. **专业模式** — 我熟悉编程与 API。请你直接用专业表述（节点类型、脚本参数、锚点、错误码等），结论优先、可附命令。
...
## 路径（二选一）
A. **先头脑风暴** — 需求还不清，先一起把目标和方案想清楚再动手  
B. **直接制作** — 目标已明确，现在就去系统里建知识库/智能体/工具

## 连接信息（三项都要）
1. 地址 HOST（浏览器打开 MaxKB 时的网址）
2. 工作空间名称或 ID
3. API Key（请用「普通用户」账号在头像 → API Key 管理里创建的 Key，不要用管理员 Key）

密钥只用于本次会话调用，不会写入 skill / 仓库文件。
```
...
3. **给 2～3 种方案** — 例如「纯 RAG / RAG+工具 / 无知识库仅工具」；标明推荐项与取舍；**智能体拓扑**从 [topology-samples](maxkb_func/maxkb-v2-workflow/topology-samples/README.md) 选型（Sequential / Parallel / ReAct / Plan…）。  
